...
class SchedulerController extends Controller
{
    public function __construct(
        private SchedulerRepository $eventRep,
    ...
    public function index(Request $r)
    {
        ...
        $events = $this->eventRep->getByNameAndType($filter, $can['events.show-system'], $can['events.show-hidden']);
    ...
    public function store(CreateRequest $r)
    {
        try {
            if ($id = $this->service->store($r->except('_token'))) {
                return redirect()
                    ->route('scheduler.edit', [$id])
                    ->with('success', 'Событие успешно сохранено. Осталось указать расписание');
            }
        } catch (\Throwable $e) {
            Log::error(
                'Ошибка при добавлении задачи планировщика '
                .json_encode($r->all()).' '.$e->getMessage()
            );
        }

        return back()->withInput($r->all())
            ->with('error', 'Ошибка при добавлении задачи планировщика');
    }

    public function edit(int $id)
    {
        $event = SchedulerTask::where('id', $id)
            ->with('points', 'eobject', 'emethod', 'escript', 'eobject')
            ->first();
        ...
        $objects = $this->objectRep->getAllToArray();
        $methods = $this->objectService->getMethodsByObjectIdToArray($event->object);
        $scripts = $this->scriptRep->getAllToArray();
        ...
        return view('scheduler.edit', compact(
            'event', 'objects', 'methods', 'scripts',
    ...
    public function update(UpdateRequest $r, int $id)
    {
        try {
            ...
            if ($this->service->update($event, $r->except('_token'))) {
                return redirect()
                    ->route('scheduler.edit', [$event->id])
                    ->with('success', 'Событие успешно изменено');
            }
        } catch (\Throwable $e) {
            Log::error(
                'Ошибка при изменении задачи планировщика '.$event->id
                .' '.json_encode($r->all()).' '.$e->getMessage()
            );
        }

        return back()->withInput($r->all())
            ->with('error', 'Ошибка при изменении задачи');
    }
}
