...
class SwitchService
{
    ...
    public function delete(int $id): bool
    {
        ...
        $this->setPort($this->portRepository
            ->getPortByObject($switch->id_object), 'button');

        Port::where('object', $switch->id_object)
            ->update(['object' => null, 'comment' => '']);
    ...
    private function setPort($idPort, $typeObject)
    {
        if ($idPort) {
            ...
            $answer = ConfigMegaService::setPortSetting(
                $port->id_device,
                $port->num_port,
                $paramsString
            );
    ...
    public function update(DeviceSwitch $switch, array $data): int
    {
        DB::transaction(function () use (&$switch, $data) {
            if ($this->isUpdateAutoObjectName($switch, $data['name'])) {
                $switch->object->name = HomeObject::getUniqueObjectName(
                    $switch->object->id,
                    trim($data['name'])
                );
        ...
        if ($data['place'] == 'Hite-pro') {
            ...
            HiteproDev::where('id_object', $switch->object->id)
                ->update(['id_object' => null]);

            Port::where('object', $switch->object->id)
