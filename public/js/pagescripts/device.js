...
function save_device_settings()
{
    ...
    dataarr['id_device'] = $("#id_device").val();
    dataarr['description'] = $("#descr_device").val();
    dataarr['ip_device'] = $("#ip_device").val();


    ajax_html(dataarr, '/savedevicesettings', '');
}
...
function new_device()
{
    ...
    dataarr['type'] = sessionStorage.getItem('typedev');
    dataarr['description'] = $("#name_device").val();
    dataarr['ip_device'] = $("#ip_device").val();
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            cost = Math.round(100/(data.totalports))-1;



            dataarr['id_device'] = data.id_new_device;

            var start_in = data.start_in;
            var end_in = data.end_in;

            //Добвляем входные порты
            for (var num = start_in; num <= end_in; num++ ){

                dataarr['num_port'] = num;
                dataarr['status'] = 'in';
                ajax_html(dataarr, '/addports', '');
            ...
            var start_out = data.start_out;
            var end_out = data.end_out;

            //Добвляем выходные порты
            for (var num = start_out; num <= end_out; num++ ){
    ...
    //Обновляем страницу с устройствами, что бы новое устройство там появилось
...
function delete_device()
{
    ...
    ajax_html(dataarr, '/deletedevice', '');
