function isEmptyInput(name) {
    return $('#lamp_form input[name='+name+']').val().trim() == '';
}
...
function validateLamp() {
    ...
    if ($('#alice_checkbox').is(':checked')){
        if (isEmptyInput('alice_command')) {
            return 'Не указано название для Алисы';
        }

        if(isEmptyAutoSelect('room'))
            return 'Не указано помещение для Алисы';
    }


    // if ($("#lamp_form input[name=type]").length && !$("#lamp_form input[name=type]:checked").val()) {
