...
@section('breadcrumbs')
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-primary">Графики: давление</h3></div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('home') }}">Главная</a></li>
                <li class="breadcrumb-item breadcrumb-item-no-link">Графики</li>
                <li class="breadcrumb-item active">Давление</li>
            </ol>
        </div>
    </div>
@endsection
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <a href="{{ route('graphs.pressures.index') }}" class="btn btn-success m-b-10 m-l-5">Обновить</a>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                @if(count($data) && (count($data['rooms']) || count($data['other_pressurestats'])))
                    @foreach($data['rooms'] as $room)
                        ...
                        @if(count($room->pressurestats))
                            @foreach($room->pressurestats as $pressurestat)
                                @include('graphs.pressures.period', compact('pressurestat'))
                                <div class="row">
                                    <div class="col col-md-12">
                                        <div id="chart{{$pressurestat->id}}" class="chartdiv"></div>
                                    </div>
                                </div>
                            @endforeach
                        @else
                            <p>Нет датчиков давления</p>
                        @endif
                        <hr>
                    @endforeach
                    @if(count($data['other_pressurestats']))
                        <h3>Остальные датчики давления</h3>
                        @foreach($data['other_pressurestats'] as $pressurestat)
                            @include('graphs.pressures.period', compact('pressurestat'))
                            <div class="row">
                                <div class="col col-md-12">
                                    <div id="chart{{$pressurestat->id}}" class="chartdiv"></div>
...
@section('scripts')
    ...
    <script>
        let url_graph = '{{ route('ajax.graphs.pressures.period.data') }}';
        ...
        $(document).ready(function(){
            ...
            function createAmChart(id, dates, values) {
                ...
                valueAxis.title.text = "Давление";

                var series = chart.series.push(new am4charts.LineSeries());
                series.dataFields.dateX = "date";
                series.dataFields.valueY = "temp";
                series.tooltipText = "Давление: [bold]{valueY}[/]";
            ...
            @foreach($data['rooms'] as $room)
                @foreach($room->pressurestats as $pressurestat)
                    $('#select_period{{$pressurestat->id}}').change();
                @endforeach
            @endforeach
