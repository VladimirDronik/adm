...
@section('breadcrumbs')
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-primary">Графики: углекислый газ</h3></div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('home') }}">Главная</a></li>
                <li class="breadcrumb-item breadcrumb-item-no-link">Графики</li>
                <li class="breadcrumb-item active">углекислый газ</li>
            </ol>
        </div>
    </div>
@endsection
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <a href="{{ route('graphs.carbdioxides.index') }}" class="btn btn-success m-b-10 m-l-5">Обновить</a>
                    </div>
                </div>
            </div>
        </div>
                ...
                @if(count($data) && (count($data['rooms']) || count($data['other_pressurestats'])))
                    @foreach($data['rooms'] as $room)
                        ...
                        @if(count($room->carbdioxides))
                            @foreach($room->carbdioxides as $carbdioxide)
                                @include('graphs.carbdioxides.period', compact('carbdioxide'))
                                <div class="row">
                                    <div class="col col-md-12">
                                        <div id="chart{{$carbdioxide->id}}" class="chartdiv"></div>
                                    </div>
                                </div>
                            @endforeach
                        @else
                            <p>Нет датчиков углекислого газа</p>
                        @endif
                        <hr>
                    @endforeach
...
@section('scripts')
    ...
    <script>
        let url_graph = '{{ route('ajax.graphs.carbdioxides.period.data') }}';
        ...
        $(document).ready(function(){
            ...
            function createAmChart(id, dates, values) {
                ...
                valueAxis.title.text = "Углекислый газ";

                var series = chart.series.push(new am4charts.LineSeries());
                series.dataFields.dateX = "date";
                series.dataFields.valueY = "temp";
                series.tooltipText = "Углекислый газ: [bold]{valueY}[/]";
