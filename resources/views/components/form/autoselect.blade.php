<div class="form-group row {{ $errors->has($name) ? ' has-error' : '' }}">
    ...
    <div class="col-sm-{{ 12 - $col }}">
        <select @if(!$multiple) id="{{ 'auto_sel_'.$name }}" @else id="{{ 'auto_sel_'.$multiple_id }}" @endif @if(isset($attributes['required'])) required @endif data-placeholder="не выбрано" name="{{ $name }}" class="chosen-select form-control" @if($multiple) multiple @endif style="width:350px;">
